...
import DocumentTypeTableToolbar from '../documentType-table-toolbar';
import DocumentTypeTableFiltersResult from '../documentType-table-filters-result';
...
export default function DocumentTypeListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.documentType.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.documentType.view(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: t('document_type'), href: paths.dashboard.documentType.root },
            { name: t('list') },
          ]}
          action={
            <Button
        ...
        <Card>
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === '1' &&
                      tableData.filter((documentType) => documentType.isActive).length}
          ...
          {canReset && (
            <DocumentTypeTableFiltersResult
              filters={filters}
              onFilters={handleFilters}
              //
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter((documentType) =>
      Object.values(documentType).some((value) =>
  ...
  if (status !== 'all') {
    inputData = inputData.filter((documentType) =>
      status === '1' ? documentType.isActive : !documentType.isActive
    );
  }
